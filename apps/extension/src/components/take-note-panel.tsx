...
export const TakeNotePanel = ({
  ...
  const [title, setTitle] = useState(page.title || "Page note");
  ...
  return (
    <section className="flex flex-1 flex-col gap-3 p-4">
      <label className="text-xs font-medium" htmlFor="repin-note-title">Title</label>
      <input
        ...
        value={title}
      />
      <label className="text-xs font-medium" htmlFor="repin-note-body">Note</label>
      <textarea
      ...
      {saved ? <p className="text-xs text-green-700" role="status">Saved to your notes</p> : null}
      <Button disabled={!title.trim() || !body.trim() || saving || saved} onClick={() => void save()}>
